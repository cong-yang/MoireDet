...
def fft_conv(
    ...
    signal_fr = torch.fft(signal_, signal_ndim=signal.ndim)
    kernel_fr = torch.fft(padded_kernel, signal_ndim=signal.ndim)

    kernel_fr.imag *= -1
    output_fr = complex_matmul(signal_fr, kernel_fr, groups=groups)
    output = torch.ifft(output_fr, signal_ndim=signal.ndim)
