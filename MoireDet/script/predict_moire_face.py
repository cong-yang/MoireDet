...
config = load_json('./real_configs/predict_moireFace.yaml')
...
def main(config):
    ...
    img_list = train_loader.dataset.img_file
    print(img_list[0])
    # return None
    ...
    with torch.no_grad():
        for i, (imgs,imgs_bak,moires, index_list) in tqdm(enumerate(train_loader)):
            ...
            for pred_moire,moire,index,img_bak,attention in zip(pred_moires,moires,index_list,imgs_bak,attentions):
                ...
                all_preds.append(np.sum(pred_moire))
                ...
                temp_img_name = img_list[index].replace('/home/users/zhenyu.yang/data/research/moire_new/identification',id_out)
                temp_dir = os.path.dirname(temp_img_name)
                if not os.path.exists(temp_dir):
                    os.makedirs(temp_dir)
                pred_moire = pred_moire[:,:,0]
                temp_img_name = temp_img_name.replace('.jpg','.png')
                temp_img_name = temp_img_name.replace('.jpeg','.png')

                cv2.imwrite(temp_img_name, pred_moire)
        ...
        for i in range(10,300,10):
            print(len([v for v in all_preds if v > i])/len(all_preds))
